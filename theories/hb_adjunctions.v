...
(** identity *)
Lemma adj_id_ {X: PO.type}: @adjunction X X types_id types_id.
Proof. done. Qed.
HB.instance Definition adj_id {X} :=
  isLeftAdjoint.Build X X types_id adj_id_. 

(** composition *)
Lemma adj_comp_ {X Y Z} (xy: X ⊣ Y) (yz: Y ⊣ Z):
  adjunction (ladj yz ∘ ladj xy) (radj xy ∘ radj yz).
Proof. move=>x y. by rewrite 2!adj. Qed.
HB.instance Definition adj_comp {X Y Z} (xy: X ⊣ Y) (yz: Y ⊣ Z) :=
  isLeftAdjoint.Build X Z (yz ∘ xy) (adj_comp_ xy yz). 

(** basic properties *)
Section s.
  Context {X Y: PO.type} (f: X ⊣ Y).
...
HB.instance Definition _ {X Y} (f: X ⊣ Y) :=
  isLeftAdjoint.Build (dual Y) (dual X) (dual_adj f) (dual_adj_ f).
...
Section s.
  ...
  HB.instance Definition _ := isMonotone.Build Y X (radj f) radj_leq.
...
HB.mixin Record isMonadic (X Y: PO.type) f of Adjunction X Y f := {
...
HB.mixin Record isCoMonadic (X Y: PO.type) f of Adjunction X Y f := {
...
Lemma monadic_id (X: PO.type): adj_monad (@types_id X) ≡ types_id.
Proof. done. Qed.
Fail HB.instance Definition _ (X: PO.type) := @isMonadic.Build X X (@types_id X) (monadic_id X).
Lemma comonadic_id (X: PO.type): adj_comonad (@types_id X) ≡ types_id.
Proof. done. Qed.
Fail HB.instance Definition _ (X: PO.type) := @isCoMonadic.Build X X (@types_id X) (comonadic_id X).
...
Lemma monadic_inf {X Y: PO.type} (f: X ·⊣ Y) P (y: Y):
...
Lemma comonadic_sup {X Y: PO.type} (f: X ⊣· Y) P (x: X):
...
(** * isomorphisms *)
HB.structure Definition Iso X Y := {i of MonadicAdjunction X Y i & isCoMonadic X Y i }.
...
Qed.
HB.instance Definition _ {X Y} (i: X ≃ Y) := isLeftAdjoint.Build Y X (i^-1) (radj_iso_ladj i).
...
(* needs the (co)monadic instances for types_id *)
Fail Check fun X: PO.type => @types_id X: X ≃ X. 
...
HB.builders Context X Y i of isIso X Y i.
  HB.instance Definition _ := isLeftAdjoint.Build X Y i fwd_bwd.
...
Fail HB.instance Definition _ {X: PO.type} :=
  @isIso.Build X X types_id types_id adj adj.
...
Definition iso_id {X: PO.type}: X ≃ X :=
  mk_iso types_id types_id adj adj. 
Definition iso_comp {X Y Z: PO.type} (i: X ≃ Y) (j: Y ≃ Z): X ≃ Z :=
  mk_iso (j ∘ i) (i^-1 ∘ j^-1) adj adj. 
